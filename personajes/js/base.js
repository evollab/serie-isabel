...
function isbController($scope, $http, $timeout, $location) {
    ...
    $scope.fns.interact = {
        ...
        activateSfera: function(personaje) { //activate a character and his related characters and draw the raphael lines
            ...
            $scope.fns.interact.clearActiveSfera(personaje);
            ...
            $("section.pastSeason .char-"+personaje.id+" h2").addClass("h2BackColor-"+personaje.esferacoronas);
        },
        activateSferaSeason3: function(personaje) { //activate a character and his related characters and draw the raphael lines
            ...
            ;
            $("section.lastSeason .char-"+personaje.id+" h2").addClass("h2BackColor-"+personaje.esferacoronas3);
        },
        ...
        clearActiveSfera: function(personaje) { //clears the active relations for the character
            ...
            $("section.pastSeason .char-"+personaje.id+" h2").removeClass("h2BackColor-"+personaje.esferacoronas);
        },
        clearActiveSferaSeason3: function(personaje) { //clears the active relations for the character
            ...
            $scope.status.charTreeHover = false;
            $("section.lastSeason .char-"+personaje.id+" h2").removeClass("h2BackColor-"+personaje.esferacoronas3);
        },
        ...
        toggleSeason: function() {
            if ($scope.status.lastSeason) {
                // Show past seasons
                $scope.fns.interact.changeSeason(false);
            } else {
                // Show last season
                $scope.fns.interact.changeSeason(true);
            }
            setTimeout(function() {
                $scope.fns.interact.drawStaticsChap();
            }, 1000);
        },
        changeSeason: function (lSeason) {
            if (lSeason) {
                // Show last season
        ...
        chooseImageCharacter: function (personaje) {
            var imgPath = '/serie-isabel/personajes/img/fotos/';
            var imgPathT3 = '/serie-isabel/personajes/img/fotos/t3/';
            if ($scope.fns.search.filterTempAct === 'primera' || $scope.fns.search.filterTempAct === 'segunda' || !personaje.initialChar3) {
                return imgPath + personaje.IMAGENES;
            } else {
                return imgPathT3 + personaje.IMAGENES3;
            }
        },
        chooseNameCharacter: function (personaje) {
            if ($scope.fns.search.filterTempAct === 'primera' || $scope.fns.search.filterTempAct === 'segunda' || !personaje.initialChar3) {
        ...
        chooseTempCharacter: function (personaje) {
            if ($scope.fns.search.filterTempAct === 'primera' || $scope.fns.search.filterTempAct === 'segunda' || !personaje.initialChar3) {
                $scope.fns.interact.changeSeason(false);
                $scope.fns.interact.clickSfera(personaje);
            } else {
                $scope.fns.interact.changeSeason(true);
                $scope.fns.interact.clickSferaSeason3(personaje);
            }
        },
    ...
    $scope.fns.preP = {
        processData: function(data) {
            ...
            for (var i = 0; i < $scope.personajes.length; i++) {
                ...
                $scope.personajes[i].safeUrl3 = $scope.fns.getUrl($scope.personajes[i]['PERSONAJE3']);
    ...
    $scope.fns.getCharByUrl = function(charUrl) {
        for (var i = 0; i < $scope.personajes.length; i++) {
            if ($scope.status.lastSeason) {
                if ($scope.personajes[i].safeUrl3 === charUrl) {
                    return $scope.personajes[i];
                }
            } else {
